...
def test_softmax(args, test_loader, model, device, dataname, glb_features):
    ...
    if glb_features == None:
        ...
        ##### 聚类特征图
        Px1, Px2, Px3, Px4 = (
            glb_features["x1"].to(device),
            glb_features["x2"].to(device),
            glb_features["x3"].to(device),
            glb_features["x4"].to(device),
        )
        Px1, Px2 = (
            Px1.reshape(-1, Px1.shape[-1]),
            Px2.reshape(-1, Px2.shape[-1]),
        )
    ...
    msg += "," + ", ".join(
        ["{}: {:.4f}".format(k, v) for k, v in zip(class_separate, vals_evaluation.avg)]
    )
    # print(msg)
    logging.info(msg)
    model.train()
...
def local_test(
    args, test_loader, model, device, dataname, glb_features, modal_mask, writer=None
):
    ...
    for i, data in enumerate(test_loader):
        ...
        if modal_mask is not None:
            mask = torch.from_numpy(np.array(modal_mask)).unsqueeze(
                0
            )  # torch.Size([1, 4])
            # mask = torch.unsqueeze(mask, dim=0).repeat(len(names), 1)
        else:
        ...
        if dataname in ["BRATS2020", "BRATS2018"]:
            ...
            if writer is not None:
                writer.writerow(
                    [
                        names[0],
                        scores_evaluation[0, 0],
                        scores_evaluation[0, 1],
                        scores_evaluation[0, 2],
                    ]
                )
        elif dataname == "BRATS2015":
            scores_separate, scores_evaluation = softmax_output_dice_class5(
...
def test_softmax_mED(args, test_loader, mask, model, device, dataname, glb_features):
    ...
    for i, data in enumerate(test_loader):
        ...
        if dataname in ["BRATS2020", "BRATS2018"]:
            scores_separate_1, scores_evaluation_1 = softmax_output_dice_class4(
                pred1, target
            )
            if writer is not None:
                writer.writerow(
            ...
            scores_separate_1, scores_evaluation_1 = softmax_output_dice_class5(
                pred1, target
            )
        ...
        if dataname in ["BRATS2020", "BRATS2018"]:
            scores_separate_2, scores_evaluation_2 = softmax_output_dice_class4(
                pred2, target
            )
        elif dataname == "BRATS2015":
            scores_separate_2, scores_evaluation_2 = softmax_output_dice_class5(
                pred2, target
            )
        ...
        if dataname in ["BRATS2020", "BRATS2018"]:
            scores_separate_3, scores_evaluation_3 = softmax_output_dice_class4(
                pred3, target
            )
        elif dataname == "BRATS2015":
            scores_separate_3, scores_evaluation_3 = softmax_output_dice_class5(
                pred3, target
            )
        ...
        for k, name in enumerate(names):
            ...
            msg += "{:>20}, ".format(name)
            vals_separate.update(
                (scores_separate_1[k] + scores_separate_2[k] + scores_separate_3[k]) / 3
            )
            vals_evaluation.update(
                (
                    scores_evaluation_1[k]
                    + scores_evaluation_2[k]
                    + scores_evaluation_3[k]
                )
                / 3
            )
            msg += ", ".join(
                [
                    "{}: {:.4f}, {:.4f}, {:.4f}".format(k, v1, v2, v3)
...
def global_test(
    test_loader, model, device, dataname="BRATS2020", feature_mask=None, mask_name=None
):
