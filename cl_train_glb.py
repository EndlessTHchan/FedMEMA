...
from dataset.datasets import Brats_train, Brats_test, GLB_Brats_train
...
if __name__ == "__main__":
    ...
    logging.info("the Brats dataset with modal {}".format(mask_name[0]))
    ...
    for epoch in range(args.num_epochs):
        ...
        if (epoch + 1) % 50 == 0:  # and (epoch+1) > 400:
            with torch.no_grad():
                msk = [True, True, True, True]
                logging.info(
                    " ########## start eval in epoch {} ########## ".format(epoch + 1)
                )
                dice_score = global_test(
                    valid_loader, model, args.device, args.dataname, mask
                )
